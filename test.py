...
dar_data = []
subject=[]
delta_indices = np.logical_and(freqs >= 0.5, freqs <= 3.9)
theta_indices = np.logical_and(freqs >= 4, freqs <= 7.9)
alpha_indices = np.logical_and(freqs >= 8, freqs <= 11.9)
...
for sub in range(len(allSubjectNumber)):
    ...
    print(isub)
    for h in range(3):
        selected_channels_good = npOut[sub, h, [TPGood_index, AFGood_index], :]
        selected_channels_bad = npOut[sub, h, [TPBad_index, AFBad_index], :]
        selected_channels_tp_good = npOut[sub, h, [TPGood_index], :]
        selected_channels_af_good = npOut[sub, h, [AFGood_index], :]
        selected_channels_tp_bad = npOut[sub, h, [TPBad_index], :]
        selected_channels_af_bad = npOut[sub, h, [AFBad_index], :]
        # Step 2: Apply delta_indices to the selected channels
        delta_power_good1 = np.sum(selected_channels_good[:, delta_indices].sum(axis=0))
        print(delta_power_good1)
        delta_power_good_tp = np.sum(selected_channels_tp_good[:, delta_indices].sum(axis=0))
        delta_power_bad_tp = np.sum(selected_channels_tp_bad[:, delta_indices].sum(axis=0))
        delta_power_good_af = np.sum(selected_channels_af_good[:, delta_indices].sum(axis=0))
        delta_power_bad_af = np.sum(selected_channels_af_bad[:, delta_indices].sum(axis=0))
        delta_power_good = np.sum(selected_channels_good[:, delta_indices].sum(axis=0))
        delta_power_bad = np.sum(selected_channels_bad[:, delta_indices].sum(axis=0))
        # Calculate power in Alpha band for good and bad conditions
        alpha_power_good_tp = np.sum(selected_channels_tp_good[:, alpha_indices].sum(axis=0))
        alpha_power_bad_tp = np.sum(selected_channels_tp_bad[:, alpha_indices].sum(axis=0))
        alpha_power_good_af = np.sum(selected_channels_af_good[:, alpha_indices].sum(axis=0))
        alpha_power_bad_af = np.sum(selected_channels_af_bad[:, alpha_indices].sum(axis=0))
        alpha_power_good = np.sum(selected_channels_good[:, alpha_indices].sum(axis=0))
        alpha_power_bad = np.sum(selected_channels_bad[:, alpha_indices].sum(axis=0))
        # Calculate power in beta band for good and bad conditions
        beta_power_good_tp = np.sum(selected_channels_tp_good[:, beta_indices].sum(axis=0))
        beta_power_bad_tp = np.sum(selected_channels_tp_bad[:, beta_indices].sum(axis=0))
        beta_power_good_af = np.sum(selected_channels_af_good[:, beta_indices].sum(axis=0))
        beta_power_bad_af = np.sum(selected_channels_af_bad[:, beta_indices].sum(axis=0))
        beta_power_good = np.sum(selected_channels_good[:, beta_indices].sum(axis=0))
        beta_power_bad = np.sum(selected_channels_bad[:, beta_indices].sum(axis=0))
        # Calculate power in theta band for good and bad conditions
        theta_power_good_tp = np.sum(selected_channels_tp_good[:, theta_indices].sum(axis=0))
        theta_power_bad_tp = np.sum(selected_channels_tp_bad[:, theta_indices].sum(axis=0))
        theta_power_good_af = np.sum(selected_channels_af_good[:, theta_indices].sum(axis=0))
        theta_power_bad_af = np.sum(selected_channels_af_bad[:, theta_indices].sum(axis=0))
        theta_power_good = np.sum(selected_channels_good[:, theta_indices].sum(axis=0))
        theta_power_bad = np.sum(selected_channels_bad[:, theta_indices].sum(axis=0))
...
excel_path = r'E:\DAR_and_power_values12.xlsx'
